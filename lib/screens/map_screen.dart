...
class _MapScreenState extends State<MapScreen> {
  ...
  Widget build(BuildContext context) {
    final bool mapSupported = kIsWeb ||
        defaultTargetPlatform == TargetPlatform.android ||
        defaultTargetPlatform == TargetPlatform.iOS;
    final List<Map<String, dynamic>> filtered = _category == 'All'
        ? _items
        : _items.where((Map<String, dynamic> item) => item['cat'] == _category).toList();
    final Map<String, dynamic>? selected = filtered.isEmpty
        ? null
        : filtered.firstWhere(
            (Map<String, dynamic> item) => item['id'] == _selectedId,
          ...
          if (mapSupported)
            GoogleMap(
              initialCameraPosition: const CameraPosition(
                target: LatLng(37.8045, -122.2712),
                zoom: 14.2,
              ),
              zoomControlsEnabled: false,
              myLocationButtonEnabled: false,
              markers: filtered.map((Map<String, dynamic> item) {
              ...
              child: SafeArea(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      children: <Widget>[
                        if (selected != null)
                          ...
                          height: 64,
                          decoration: BoxDecoration(
                            color: (selected['color'] as Color).withValues(alpha: 0.16),
                            borderRadius: BorderRadius.circular(14),
                            border: Border.all(
                              color: (selected['color'] as Color).withValues(alpha: 0.22),
                            ),
                          ),
                          alignment: Alignment.center,
                          child: Text(
                              ...
                              : Column(
                                  ...
                                  children: <Widget>[
                                    Row(
                                      children: <Widget>[
                                        Text(
                                          (selected['open'] as bool) ? 'OPEN' : 'CLOSED',
                                          style: TextStyle(
                                            fontSize: 11,
                                            letterSpacing: 0.4,
                                            fontWeight: FontWeight.w700,
                                            color: (selected['open'] as bool)
                                                ? AppColors.openGreen
                                                : AppColors.inkFaint,
                                          ),
                                        ),
                                        Text(
                                          ' · until ${selected['close']}',
                                    ...
                                    Text(
                                      '${selected['rating']} · ${selected['reviews']} reviews · ${selected['price']} · ${selected['dist']}',
                                      style: const TextStyle(
                                        fontSize: 13,
                                        color: AppColors.inkSoft,
                                      ),
                        ...
                        if (selected != null)
                          ...
                          onPressed: () {
                            setState(() {
                              if (_saved.contains(selected['id'] as int)) {
                                _saved.remove(selected['id'] as int);
                              } else {
                                _saved.add(selected['id'] as int);
                              }
                            });
                          },
                          icon: Icon(
                            _saved.contains(selected['id'] as int)
                                ? Icons.favorite_rounded
                                : Icons.favorite_border_rounded,
                            color: _saved.contains(selected['id'] as int)
                                ? AppColors.primary
                                : AppColors.inkSoft,
                          ),
                        ),
                    ...
                    OutlinedButton(
                      onPressed:
                          selected == null ? null : () => context.push('/business/${selected['id']}'),
                      child: const Row(
