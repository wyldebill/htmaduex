...
ENV_ARG="${1:-.env}"
ENV_PATH="$ENV_ARG"
if [[ ! -f "$ENV_PATH" && "$ENV_ARG" != /* ]]; then
  ENV_PATH="$REPO_ROOT/$ENV_ARG"
fi

if [[ ! -f "$ENV_PATH" ]]; then
  echo "Missing $ENV_ARG. Add required Maps/Firebase keys first." >&2
  exit 1
fi
...
optional_secret() {
  local key="$1"
  local value
  value="$(grep -E "^[[:space:]]*${key}=" "$ENV_PATH" | head -n1 | cut -d'=' -f2- | tr -d '"' | tr -d "'" || true)"
  echo "$value"
}
...
firebase_project_id="$(optional_secret FIREBASE_PROJECT_ID)"
firebase_auth_domain="$(optional_secret FIREBASE_AUTH_DOMAIN)"
firebase_storage_bucket="$(optional_secret FIREBASE_STORAGE_BUCKET)"
firebase_ios_bundle_id="$(optional_secret FIREBASE_IOS_BUNDLE_ID)"
...
EOF
echo "Generated $REPO_ROOT/ios/Flutter/Secrets.xcconfig"
